...
def processar_conteudo(estrutura, pdf_reader, pasta_base):
    """
    Função recursiva para navegar no JSON.
    Lida com estruturas simples ou aninhadas (Seções -> Capítulos).
    """
    for titulo, valor in estrutura.items():
        if isinstance(valor, list):
            # Caso base: É um capítulo com intervalo de páginas [inicio, fim]
            inicio, fim = valor
            nome_arquivo = limpar_nome_arquivo(titulo) + ".txt"
            ...
            # Caso recursivo: É uma seção (ex: "Large Language Models" do Jurafsky)
            nome_subpasta = limpar_nome_arquivo(titulo)
            nova_pasta_base = os.path.join(pasta_base, nome_subpasta)
            
            if not os.path.exists(nova_pasta_base):
                os.makedirs(nova_pasta_base)
            
            print(f" Entrando na seção: {titulo}")
            processar_conteudo(valor, pdf_reader, nova_pasta_base)

def main():
    pasta_saida_principal = "livros_extraidos"
    if not os.path.exists(pasta_saida_principal):
        os.makedirs(pasta_saida_principal)
    ...
    for caminho_pdf, conteudo in biblioteca_json.items():
        ...
        # Cria pasta para o livro específico
        nome_pasta_livro = limpar_nome_arquivo(caminho_limpo.replace(".pdf", ""))
        caminho_pasta_livro = os.path.join(pasta_saida_principal, nome_pasta_livro)
        
        if not os.path.exists(caminho_pasta_livro):
            os.makedirs(caminho_pasta_livro)

        try:
            reader = PdfReader(caminho_limpo)
            processar_conteudo(conteudo, reader, caminho_pasta_livro)
        except Exception as e:
            print(f"Erro crítico ao abrir o PDF {caminho_limpo}: {e}")

    print("\n--- Processo Finalizado! Verifique a pasta 'livros_extraidos' ---")
